...
Hệ thống AI hỗ trợ gồm nhiều **AI Agent** (mỗi Agent = một trợ lý độc lập) do các nhóm khác nhau phát triển, và 1. **AI Orchestrator** điều phối
Mỗi **AI Agent**:
...
│ │ Agent 3 – Viết tài liệu      │ │   │ - Multi-agent flow   │    │                |  
...
```python
{
  ...
  context: {
    ...
    history: [
      ...
      {
        role: "assistant",
        content: "Chào bạn! Mình đã nhận được tin nhắn test. Bạn cần hỗ trợ gì không?",
      },
    ],
  },
}
...
Hệ thống sử dụng **MinIO** để lưu trữ các file, tài liệu phục vụ cho các AI Agent và dự án.
...
Hệ thống đã triển khai **Ollama** cùng khoảng **15 mô hình LLM** trên các server nội bộ, đồng thời cấu hình **Proxy API** để các nhóm có thể **gọi API thống nhất**, không cần quan tâm mô hình đang chạy ở đâu.
...
Hệ thống đã triển khai **Ollama** cùng khoảng **15 mô hình LLM** trên các server nội bộ, đồng thời cấu hình **Proxy API** để các thầy cô và nhóm  có thể **gọi API thống nhất**, không cần quan tâm mô hình đang chạy ở đâu.
...
* Phù hợp để tích hợp vào Agent, Orchestrator hoặc script
...
* Phục vụ RAG, tìm kiếm ngữ nghĩa, clustering dữ liệu 


## 12. API tổng hợp: Tóm tắt & Embedding tài liệu

Hệ thống cung cấp endpoint **kết hợp xử lý tóm tắt và sinh embedding**, phục vụ trực tiếp cho nhu cầu xử lý tài liệu (PDF, DOCX, …).
...
## 13. Data Lake – Kiến trúc lưu trữ & quản trị dữ liệu

### 13.1 Mục tiêu của Data Lake

**Data Lake** trong hệ thống là **kho lưu trữ trung tâm** trên **NAS Synology**, phục vụ cho toàn bộ vòng đời dữ liệu:

* Lưu trữ **bản gốc tài liệu** (PDF, DOCX, dữ liệu thô)
