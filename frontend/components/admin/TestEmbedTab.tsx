...
import { MessageCircle, RefreshCw, BookOpen, ChevronDown, ChevronUp } from "lucide-react"
...
export function TestEmbedTab() {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-220px)] min-h-[400px] gap-4">
      {/* Hướng dẫn nhúng - Step by Step */}
      <Collapsible open={guideOpen} onOpenChange={setGuideOpen} className="w-full">
        <Card>
          <CollapsibleTrigger asChild>
            <CardHeader className="cursor-pointer hover:bg-muted/50 transition-colors rounded-t-lg flex flex-row items-center justify-between space-y-0 pb-2">
              <div className="flex items-center gap-2">
                <BookOpen className="h-5 w-5 text-primary" />
                <div>
                  <CardTitle className="text-base">Hướng dẫn nhúng (Embed) Trợ lý AI</CardTitle>
                  <CardDescription>Step-by-step cho người muốn nhúng widget vào website hoặc ứng dụng</CardDescription>
                </div>
              </div>
              {guideOpen ? <ChevronUp className="h-5 w-5 shrink-0 opacity-50" /> : <ChevronDown className="h-5 w-5 shrink-0 opacity-50" />}
            </CardHeader>
          </CollapsibleTrigger>
          <CollapsibleContent>
